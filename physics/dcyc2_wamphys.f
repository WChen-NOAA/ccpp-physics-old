...
!> @{
      subroutine dcyc2t3_run(do_wamphys, me, master, kdt,               &
!  ---  inputs:
     &       solhr,slag,sdec,cdec,sinlat,coslat,                        &
...
!     logical lprnt
     ...
     &      sfcnirbmd, sfcnirdfd, sfcvisbmd, sfcvisdfd
      ...
      real(kind=kind_phys), dimension(:,:), intent(inout) :: swh, hlw   
...
!      note: sfc emiss effect will not be appied here
      ...
      enddo
!           if(do_wamphys) print *, ' dcyc2_wamphys only xmu'  
...
!	   print*, 'dcyc2_hlw  ', maxval(hlw)*86400,  minval(hlw)*86400	   
!	 endif 
