...
!>  @{
      ...
      & index_of_y_wind, index_of_process_wam_phys, ntoz,nto1,nto2, do_wamphys_diag,errmsg, errflg)
      ...
      integer, intent(in) :: index_of_temperature, index_of_x_wind, index_of_y_wind, index_of_process_wam_phys
      ...
      if (lssav) then
        ...
        if (ldiag3d .and. flag_for_wam_generic_tend .and. .not.do_wamphys_diag) then
          idtend = dtidx(index_of_temperature, index_of_process_wam_phys)
          if(idtend>=1) then
            dtend(:,:,idtend) = dtend(:,:,idtend) + dtdt*dtf
          endif

          idtend = dtidx(index_of_x_wind, index_of_process_wam_phys)
          if(idtend>=1) then
            dtend(:,:,idtend) = dtend(:,:,idtend) + dudt*dtf
          endif

          idtend = dtidx(index_of_y_wind, index_of_process_wam_phys)
          if(idtend>=1) then
            dtend(:,:,idtend) = dtend(:,:,idtend) + dvdt*dtf
          endif
        endif
        if (qdiag3d .and. flag_for_wam_generic_tend) then
          idtend = dtidx(100+ntoz, index_of_process_wam_phys)
          if(idtend>=1) then
            dtend(:,:,idtend) = dtend(:,:,idtend) + dqdt(:,:,ntoz)*dtf
          endif

          idtend = dtidx(100+nto1, index_of_process_wam_phys)
          if(idtend>=1) then
            dtend(:,:,idtend) = dtend(:,:,idtend) + dqdt(:,:,nto1)*dtf
          endif

          idtend = dtidx(100+nto2, index_of_process_wam_phys)
          if(idtend>=1) then
